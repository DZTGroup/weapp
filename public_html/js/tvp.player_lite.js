...
var tvp=window.tvp||{};var TenVideoPlayer=window.tvp;tvp.lastModify="$Date: 2013-08-27 14:02:31 +0800 (二, 2013-08-27) $";tvp.ver="$Rev: 33882 $";tvp.lastChangedBy="$LastChangedBy: popotang $";tvp.path={base:"http://qzs.qq.com/tencentvideo_v1/js/tvp/",hls_video_cgi:"http://vv.video.qq.com/gethls?otype=json&format=2",html5_video_cgi:"http://vv.video.qq.com/geturl?otype=json"}
...
return;if(!!flashobj&&tvp.$.isFunction(flashobj.getPlaytime)&&tvp.$.isFunction(flashobj.pauseVideo)){pauseTime=flashobj.getPlaytime();flashobj.pauseVideo();$me.isStartPlay=false;pauseCheckTimer=setInterval(function(){try{if(!flashobj||!tvp.$.isFunction(flashobj.getPlaytime)){clearInterval(pauseCheckTimer);pauseCheckTimer=null;}else if(flashobj.getPlaytime()!=pauseTime){clearInterval(pauseCheckTimer);pauseCheckTimer=null;$me.isStartPlay=true;}}catch(err){if(!!pauseCheckTimer){clearInterval(pauseCheckTimer);pauseCheckTimer=null;}}},50);}}
...
function getPlatform(){var bussId=getBusinessId(),deviceId=getDeviceId();if(bussId==6&&tvp.$.userAgent.windows){if(Math.random()<0.1){var param={"cmd":"3003","url":document.location.href,"ua":navigator.userAgent,"str1":"windows"};tvp.report(param);}}
return bussId*10000+deviceId*100+1;}
function report(step,val,extData){if(isNaN(val)||val<=0||val>9000000){return;}
var url="http://rcgi.video.qq.com/report/play?";var r=[],pa={"step":step,"val":val,"ptag":tvp.$.cookie.get("ptag"),"version":"TenPlayerHTML5V1.0","ctime":tvp.$.getISOTimeFormat(),"vid":curVid,"rid":reportRid,"pid":reportPid};if(typeof extData=="object"){tvp.$.extend(pa,extData);}
tvp.$.extend(pa,videodata);for(var p in pa){var v=pa[p];if(isNaN(v)){v=encodeURIComponent(""+v);}
r.push(p+"="+v);}
url+=r.join("&");tvp.report(url);}
function getVideoUrlByVid(vid,hls,callbacks){function error(errCode){if(hls){getVideoUrlByVid(vid,false,callbacks);}else if(tvp.$.getType(callbacks)=="object"&&tvp.$.isFunction(callbacks.onError)){callbacks.onError(errCode);}
...
if(!tvp.$.isString(vid))
return;var isPay=curVideo.getPay()>0?1:0;reportRid=tvp.$.createGUID(48);reportPid=tvp.$.createGUID(48);reportTimer["step_1009"]=new TimerObject();var reportData={val1:0,val2:0},ajaxTimelong=0;tvp.$.ajax({"url":(hls&&!isPay)?tvp.path.hls_video_cgi:tvp.path.html5_video_cgi,"data":{"vid":vid,"charge":curVideo.getPay()>0?1:0},"dataType":"jsonp","success":function(json){ajaxTimelong=reportTimer["step_1009"].getTimelong();if(!json||!json.s){error(50);return;}else if(json.s!="o"){error(json.em||50);return;}else if(!json.vd||!json.vd.vi||!tvp.$.isArray(json.vd.vi)){error(68);return;}
...
curVid=vid;reportData["val1"]=1;report((hls&&!isPay)?1010:1009,ajaxTimelong,reportData);if(tvp.$.getType(callbacks)=="object"&&tvp.$.isFunction(callbacks.onSuccess)){callbacks.onSuccess(json,videourl);}},"error":function(){ajaxTimelong=reportTimer["step_1009"].getTimelong();error(500);}});}
function getVideoTagHtml(){playerid=$me.params.playerid;if(!playerid){playerid="tenvideo_video_player_"+(tvp.$("video").size());}
...
if(Math.random()<0.1){tvp.report({"cmd":"2551","type":1,"host":h,"url":document.location.href,"ua":navigator.userAgent,"ver":"$Rev: 33882 $","str1":playername,"int1":supportMP4?1:0});}}
...
tvp.Player.prototype=new tvp.BasePlayer();tvp.Player.instance=[];/*  |xGv00|1fc6305fc9934826403618ce42747e0e */