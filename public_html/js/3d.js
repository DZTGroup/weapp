...
FCAPP.HOUSE.FULL3D = {
    ...
    init: function() {
        var R = FULL3D.RUNTIME,
            url = 'http://cgi.trade.qq.com/cgi-bin/common/wx_redirect.fcg?appid=';
        ...
        if (window.gQuery && gQuery.appid) {
            url += gQuery.appid;
            window.shareData.link = url + encodeURIComponent(window.shareData.link);
            window.shareData.linkKeep = url + encodeURIComponent(window.shareData.linkKeep);
        }
    ...
    back2List: function() {
        if (/Mac OS/i.test(navigator.userAgent)) {
            ...
            var url = 'apartment.html' + location.search.replace('&houseid=' + gQuery.houseid, '');
            location.href = url;
    ...
    showRooms: function(data) {
        ...
        if (data.rooms) {
            ...
            if (full3d.length > 0) {
                ...
                FULL3D.renderBgImg(full3d[0].bimg);
                R.full3d = full3d;
                return;
            }
        }
