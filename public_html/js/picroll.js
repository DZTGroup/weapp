...
FCAPP.HOUSE.Picroll = {
    ...
    RUNTIME:{findCategory:false},
    init:function(){
        var R = Picroll.RUNTIME;
        if(!R.binded){
            R.binded = true;         //事件只绑定一次
            Picroll.initElements(R);
            Picroll.bindEvents(R);
            R.detailState = 'hide';  //详情状态默认收起
            R.zoomed = false;        //当前是否有图片被放大
            R.template = FCAPP.Common.escTpl($('#template').html());  //模板
            R.imgDom = [];
    ...
    resizeLayout:function(){
        ...
        R.h = document.documentElement.clientHeight;
        t *= R.w;
        R.picTank.css({width:t+'px',height: R.h+'px'});
    ...
    showRooms:function(res){
        ...
        if(idx == -1){
            FCAPP.Common.msg(true,{msg:'没找到该户型'});
        }else{
            find = true;
    ...
    renderPics: function(room){
        ...
        detail=$.template(R.template,{data:room});
        ...
        R.loadSize.push({w:room.width,h:room.height});

        for(var i=0,il=pics.length;i<il;i++){
            ...
            R.loadSize.push({w:pics[i].width,h:pics[i].height});
        }
    ...
    sendMsgResult:function(res){
        if(res.ret == 0){
            FCAPP.Common.msg(true,{msg:'已发送这张图片给你的微信'});
        }else{
            FCAPP.Common.msg(true,{msg:'保存失败，请稍后尝试'});
        }
    },
    zoomPic:function(){
        var R = Picroll.RUNTIME,
    ...
    zoomIn:function(R,timeout){
        ...
        //R.downBtn.hide();
        R.detailCav.hide();
        FCAPP.Common.loadImg(src,'zoomImg',function(img){  
            if(idx != 0){
                img.width = w;
                img.height = h;
            }else{
            ...
            R.downBtn.hide();
            R.zoomBtn.hide();
            R.zoomDiv.css({width:img.width+'px',height:img.height+'px'});
            setTimeout(function(){
    ...
    zoomOut:function(R,timeout){
        ...
        myScroll.refresh();
        setTimeout(function(){
            R.zoomDiv.css({width:'100%',height:'100%'});
            R.downBtn.show();
            R.zoomBtn.show();
    ...
    loadedImgProcess:function(img){
        ...
        if(R.isWeixin){ img.onclick = Picroll.showBtn;}
        //console.log(R.w, R.h,img.idx,img.width,img.height,style);
    }
};
